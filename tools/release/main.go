...
var goModRequireSDKRegexp = regexp.MustCompile(`github.com/nholuongut/tflint-plugin-sdk v(.+)`)
var goModRequireBundledPluginRegexp = regexp.MustCompile(`github.com/nholuongut/tflint-ruleset-terraform v(.+)`)
...
func checkGitStatus() error {
	...
	if !strings.Contains(strings.TrimSpace(stdout.String()), "nholuongut/tflint") {
		return fmt.Errorf("remote.origin is not nholuongut/tflint, got %s", strings.TrimSpace(stdout.String()))
	}
	return nil
}
