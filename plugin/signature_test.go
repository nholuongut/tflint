...
func Test_GetSigningKey(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			Config:   NewInstallConfig(tflint.EmptyConfig(), &tflint.PluginConfig{SigningKey: "", SourceOwner: "nholuongu"}),
			Expected: builtinSigningKey,
		},
		{
			Name:     "bulit-in signing key and configured signing key",
			Config:   NewInstallConfig(tflint.EmptyConfig(), &tflint.PluginConfig{SigningKey: testSigningKey, SourceOwner: "nholuongu"}),
			Expected: testSigningKey,
		},
	}
