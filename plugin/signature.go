...
// If the plugin is under the nholuongu organization, you can use the built-in key even if the signing_key is omitted.
func (c *SignatureChecker) GetSigningKey() string {
	if c.config.SigningKey != "" {
		return c.config.SigningKey
	}
	if c.config.SourceOwner == "nholuongu" {
...
// builtinSigningKey is the default signing key that applies only to plugins under the nholuongu organization.
